...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from OdysseyRom Github (http://github.com/OdysseyRom)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/OdysseyRom/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/odyssey_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'OdysseyRom/%s already exists' % (repo_name)
            else:
    ...
    f = open('.repo/local_manifests/odyssey_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for OdysseyRom/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: OdysseyRom/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "OdysseyRom/%s" % repo_name, "revision": "jb4.3" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/odyssey.dependencies'
    syncable_repos = []
...
print "Repository for %s not found in the OdysseyRom Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/odyssey_manifest.xml" % device
